...
def create_index(filenames, index, file_titles):
    ...
    for filename in filenames:
        with open(filename) as f:
            ...
            add_terms_to_index(title, index, filename)
            for line in f:
                add_terms_to_index(line, index, filename)

def add_terms_to_index(line, index, filename):
    """
    This function is passed:
        line:       a string of terms
        index:      an index dictionary
        filename:   name of the file that contains the line
    
    The function will parse out all the terms in line in the normalized form
    and update the index dictionary. 
...
def search(index, query):
    ...
    for term in query.split():
        ...
        if term not in index:
            return []
        docs_list.append(set(index[term]))
    return list(set.intersection(*docs_list))
