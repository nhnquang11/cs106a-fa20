...
This program emulates falling sand with Brownian motion.
"""
...
def check_move(grid, x1, y1, x2, y2):
    ...
    False
    """
    # The destination must be within the edges of the grid
    if not in_bound(x2, y2, grid):
        return False

    # The destination square in the grid must be empty
    if grid[y2][x2]:
        return False

    return available_move(x1, y1, x2, y2, grid)

def available_move(x1, y1, x2, y2, grid):
    """
    Checks whether move from (x1, y1) to (x2, y2) is one of the following moves:
    1 - Left
    2 - Right
    3 - Down
    4 - Down Left
    5 - Down Right
    Assumes that the destination is in bound.
    """
    # Left or Right
    if abs(x1 - x2) == 1 and y1 == y2:
        return True
    # Down Left or Down Right (the corner square must be empty)
    elif abs(x1 - x2) == 1 and y2 - y1 == 1:
        return not grid[y1][x2]
    # Down
...
def in_bound(x, y, grid):
    """
    Checks whether (x, y) is in the grid or not.
    """
...
def do_whole_grid(grid, brownian):
    ...
    """
    # Traverse the rows in reverse order to avoid doing gravity multiple times
