...
File: ghost.py
--------------
This program takes in multiple images at the same position
and generates a new image with some "clutter" removed.
"""
...
def get_best_pixel(pixel_list):
    ...
    (255, 0, 0)
    """
    red_avg = sum([pixel.red for pixel in pixel_list]) / len(pixel_list)
    green_avg = sum([pixel.green for pixel in pixel_list]) / len(pixel_list)
    blue_avg = sum([pixel.blue for pixel in pixel_list]) / len(pixel_list)

    best_pixel = pixel_list[0]
    best_dist = get_pixel_dist(pixel_list[0], red_avg, green_avg, blue_avg)
...
def create_ghost(image_list):
    ...
    """
    ghost_image = SimpleImage.blank(width=image_list[0].width, height=image_list[0].height)
    for y in range(ghost_image.height):
        for x in range(ghost_image.width):
            pixel_list = [image.get_pixel(x, y) for image in image_list]
